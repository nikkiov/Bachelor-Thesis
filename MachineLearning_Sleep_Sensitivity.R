...
circadian_data_plot_percent <- circadian_data_plot %>%
  group_by(sleep_problems, variable) %>%
  mutate(total = n()) %>%
  group_by(sleep_problems, variable, value_label) %>%
  summarise(count = n(), 
            total = first(total)) %>%
  ungroup() %>%
...
circadian_data_plot_percent %>%
  ggplot(aes(x = value_label, y = percentage, fill = sleep_problems)) +
  ...
  labs(fill = "Depression", x = NULL, y = "Percentage (%)", title = "Percentage distribution of covariates by sleep disturbances group (sensitivity dataset)") +
